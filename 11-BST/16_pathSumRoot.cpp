...
void pathSum_FromRoot(BinaryTreeNode<int>* root, string s,int sum)
{
    if(root==NULL) return;
    ...
    sum-= root->data;
    if(sum== 0  &&  root->left==NULL && root->right==NULL) cout<<s<<endl;
    else if(sum>0)
    {
        pathSum_FromRoot(root->left,s,sum);
        pathSum_FromRoot(root->right,s,sum);
    }
