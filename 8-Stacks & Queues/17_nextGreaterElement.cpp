...
int main(){
	    ...
	    for(int i=n-2;i>=0;i--)
	    {
	        ...
	        if(i==0) vi.push_back(-1);  // Bcoz its given in question that last element should have "-1" coz there's nothing greater than that on the right
	        if(st.empty()) 
	        {
	            st.push(arr[i]);
                vi.push_back(-1);
